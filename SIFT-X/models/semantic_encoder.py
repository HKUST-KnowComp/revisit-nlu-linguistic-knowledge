...
class SemanticEncoder(BaseLightningModel):
    def __init__(self, args):
        ...
        pytorch_total_params = sum(p.numel() for p in self.transformer.parameters() if p.requires_grad)
#         logging.info("Number of activate parameters: %s", pytorch_total_params)
    ...
    def compute_loss(self, output_dict, labels, batch_idx, model):
    ...
    def pool_node_embeddings(self, last_layers, masks, gdata, batch_num_nodes):
        ...
        node_embeddings_mask = torch.arange(max(batch_num_nodes), device=device).expand(bsz, -1) < batch_num_nodes.clone().detach().unsqueeze(1)
    ...
    def prepare_data(self):
        ...
        logging.info("Creating features from dataset file at %s", args.data_dir) # logger --> logging
        examples = {
        ...
        logging.info("Saving features into cached file %s", cached_features_file) # logger --> logging
    ...
    def load_dataset(self, mode, batch_size, shuffle=False, num_workers=0, pin_memory=False):
        "Load datasets. Called after prepare data."

        if not hasattr(self, 'data_features'):
            logging.info('Loading feature files') # logger --> logging
            self.data_features = pickle.load(open(self._feature_file(), 'rb'))
#         if mode == 'train':
#             n = round(len(self.data_features[mode]) / 5)
#             features = random.sample(self.data_features[mode], n)
#         else:
    ...
    def _model_step(self, model, batch, batch_idx, compute_loss=False):
        inputs = {
            "input_ids": batch[0],
        ...
        if compute_loss:
            output += (self.compute_loss(output_dict, labels, batch_idx, model),)
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        _, _, loss = self._model_step('train', batch, batch_idx, compute_loss=True)
        self.log("loss", loss, on_step=True, on_epoch=False, logger=True)

        return {"loss": loss} #"rate": self.lr_scheduler.get_last_lr()[-1]            
        
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        output_dict, labels, loss = self._model_step('validate', batch, batch_idx, compute_loss=True)
        output_dict = self.recursively_detach(output_dict)
        labels = self.recursively_detach(labels)
        return {"val_loss": loss, "output_dict": output_dict, "target": labels}

    def test_step(self, batch, batch_idx, dataloader_idx=0):
        output_dict, _ = self._model_step('test', batch, batch_idx)
        output_dict = self.recursively_detach(output_dict)
