...
class BaseLightningModel(pl.LightningModule):
    ...
    def train_dataloader(self, shuffle=True, use_lr_scheduler=True):
        ...
        dataloader = self.load_dataset("train", train_batch_size, shuffle=shuffle, num_workers=self.args.num_workers)
    ...
    def validation_epoch_end(self, outputs: list) -> dict:
        ...
        self.log("val_loss", logs["val_loss"], logger=True)
        if not self.trainer.sanity_checking:
            for m in self.metric_to_watch:
                self.log(m, logs[m], logger=True)
