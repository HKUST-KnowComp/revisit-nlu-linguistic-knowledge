...
from models.pretrained_transformer import PretrainedTransformer
...
def all_rdf_to_dgl(examples, tokenizer, max_length, all_split_names, all_rdf_graphs, all_metadata, graph_type, bidirectional=True):
    ...
    for split, rdf_graphs, metadata in zip(all_split_names, all_rdf_graphs, all_metadata):
        ...
        for rdf_graph, mdata in zip(rdf_graphs, metadata):
            count += 1
            graph = rdf2dgl(
                rdf_graph, mdata, relation2id, graph_type, te, bidirectional=bidirectional
            )
            graphs[split].append(graph['G'])
            sub_graph.append(graph['G'])
            if graph_type == "skeleton":
                count_all += graph['Total_edge']
                overlap_all += graph['Select_edge']
                count_node_all += graph['Total_node']
                select_node += graph['Select_node']
            elif graph_type == "parsed":
                count_all += graph['Total_edge']
                count_node_all += graph['Total_node']
                select_node += graph['Select_node']
                for key, value in graph['Edge_type'].items():
                    try:
                        edge_relation[key] += value
                    except:
                        edge_relation[key] = value
            else:
                count_all += graph['Total_edge']
        ...
        edge_aver = {}
        edge_aver_per_node = {}
        for key, value in edge_relation.items():
            edge_aver[key] = round(value / count, 4)
            edge_aver_per_node[key] = round(value / count_node_all , 4)
        if graph_type == "skeleton":
            logging.info("Mode: %s", split)
            logging.info("Total edges: %s", count_all)
            logging.info("Select edges: %s", overlap_all)
            logging.info("Total nodes: %s", count_node_all)
            logging.info("Select nodes: %s", select_node)
            logging.info("Average edges: %s", round(overlap_all / count, 4))
            logging.info("Average edges per node: %s", round(overlap_all / select_node, 4))
            logging.info("Average nodes: %s", round(select_node / count, 4))
        elif graph_type == "parsed":
            logging.info("Mode: %s", split)
            logging.info("Total edges: %s", count_all)
            logging.info("Total nodes: %s", count_node_all)
            logging.info("Select nodes: %s", select_node)
            logging.info("Average edges: %s", round(count_all / count, 4))
            logging.info("Average edges per node: %s", round(count_all / count_node_all, 4))
            logging.info("Edge type: %s", edge_relation)                               
            logging.info("Average edge per graph: %s", edge_aver)
            logging.info("Average edge per node: %s", edge_aver_per_node)
            logging.info("Average nodes: %s", round(count_node_all / count, 4))
        else:
            logging.info("Mode: %s", split)
            logging.info("Total edges: %s", count_all)
            logging.info("Select_edges: %s", overlap_all)
            logging.info("Total nodes: %s", count_node_all)
            logging.info("Select nodes: %s", select_node)
            logging.info("Edge type: %s", edge_relation)            
            logging.info("Average edges: %s", round(count_all / count, 4))
...
def get_graphs(
    ...
    if "QQP" in data_dir:
        del train_rdf_graphs[497656]
        del train_rdf_graphs[497657]
        del train_rdf_graphs[620242]
        del train_rdf_graphs[620243]
        del train_rdf_graphs[724450]
        del train_rdf_graphs[724451]
        del train_metadata[497656]
        del train_metadata[497657]
        del train_metadata[620242]
        del train_metadata[620243]
        del train_metadata[724450]
        del train_metadata[724451]
    ...
    return all_rdf_to_dgl(examples, tokenizer, max_length, all_split_names, all_rdf_graphs, all_metadata, graph_type)
